class Clock5 {
    ...
    updateClock5() {
      ...
      requestAnimationFrame(() => this.updateClock5());
    }
  
    initializeClock5() {
      this.clock5El.innerHTML = `<svg class="clock5face" width="300" height="300" viewBox="-150 -150 300 300">
      ...
      this.UI.date = this.clock5El.querySelector('.date');
      this.UI.am_pm = this.clock5El.querySelector('.am-pm');
      this.UI.second = this.clock5El.querySelector('.hand--second');
      this.UI.minute = this.clock5El.querySelector('.hand--minute');
      this.UI.hour = this.clock5El.querySelector('.hand--hour');
  
      requestAnimationFrame(() => this.updateClock5());
    }
  }
  
  const clock5s = document.querySelectorAll('.clock5');
  clock5s.forEach(el => new Clock5(el))
