class Clock4 {
    ...
    updateClock4() {
      ...
      requestAnimationFrame(() => this.updateClock4());
    }
  
    initializeClock4() {
      this.clock4El.innerHTML = `<svg class="clock4face" width="300" height="300" viewBox="-150 -150 300 300">
      ...
      this.UI.date = this.clock4El.querySelector('.date');
      this.UI.am_pm = this.clock4El.querySelector('.am-pm');
      this.UI.second = this.clock4El.querySelector('.hand--second');
      this.UI.minute = this.clock4El.querySelector('.hand--minute');
      this.UI.hour = this.clock4El.querySelector('.hand--hour');
  
      requestAnimationFrame(() => this.updateClock4());
    }
  }
  
  const clock4s = document.querySelectorAll('.clock4');
  clock4s.forEach(el => new Clock4(el))
