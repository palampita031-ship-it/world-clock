class Clock3 {
    ...
    updateClock3() {
      ...
      requestAnimationFrame(() => this.updateClock3());
    }
  
    initializeClock3() {
      this.clock3El.innerHTML = `<svg class="clock3face" width="300" height="300" viewBox="-150 -150 300 300">
      ...
      this.UI.date = this.clock3El.querySelector('.date');
      this.UI.am_pm = this.clock3El.querySelector('.am-pm');
      this.UI.second = this.clock3El.querySelector('.hand--second');
      this.UI.minute = this.clock3El.querySelector('.hand--minute');
      this.UI.hour = this.clock3El.querySelector('.hand--hour');
  
      requestAnimationFrame(() => this.updateClock3());
    }
  }
  
  const clock3s = document.querySelectorAll('.clock3');
  clock3s.forEach(el => new Clock3(el))
